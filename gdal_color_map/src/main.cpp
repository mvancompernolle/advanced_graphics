...
bool initialize();
void cleanUp();
void drawText(std::string text, float x, float y, float color[]);
...
void render()
{
	...
	// render meshes
	terrain->render(loc_position, loc_normal, loc_texture);
...
void reshape(int n_w, int n_h)
{
    ...
    projection = glm::perspective(45.0f, float(w)/float(h), 0.01f, drawDistance);
...
void keyboardUp(unsigned char key, int x_pos, int y_pos){
	switch(key)
	{
		case ' ': // Space Bar to start and stop rotate
			//keyArr[int(' ')] = NOTPUSHED;
			break;
	}
}
...
bool initialize()
{
	...
	// get color map texture for terrain
	...
	if(!vertShader.loadShader("../bin/shaders/vertShader.vs") || !
	fragShader.loadShader("../bin/shaders/fragShader.fs")){
    ...
    /*loc_specularStrength = glGetUniformLocation(program,
                    const_cast<const char*>("specularStrength"));
    if(loc_specularStrength == -1)
    {
        std::cerr << "[F] SPECULAR STRENGTH NOT FOUND" << std::endl;
        return false;
    }

    loc_matSpecularIntensity = glGetUniformLocation(program,
                    const_cast<const char*>("specularIntensity"));
    if(loc_matSpecularIntensity == -1)
    {
        std::cerr << "[F] SPECULAR STRENGTH NOT FOUND" << std::endl;
        return false;
    }*/
...
void drawText(std::string text, float x, float y, float color[]){
	...
	// draw text to indicate rotation direction
	glRasterPos2f(x, y);
