...
void Terrain::setDataZoneData(const char* fileName){
	...
	Texture* dataZoneData = new Texture(GL_TEXTURE_2D, fileName);
	dataZoneData->load();
	dataset = dataZoneData->getGDALImage();
...
bool Terrain::generateMesh(){
	...
	if(gdalDataZone != NULL){
		...
		std::cout << gdalDataZone->GetRasterCount() << std::endl;
	...
	for(int y=-yOffset; y<height-yOffset-1; y++){
		...
		if(gdalDataZone != NULL){
			//std::cout << "didn't break " << y + yOffset << std::endl; 
